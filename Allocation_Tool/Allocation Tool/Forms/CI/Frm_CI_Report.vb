...
Public Class Frm_CI_Report
    ...
    Private Sub refreshData()
        ...
        If Category.Length > 0 Then
            temp = Split(Category, ",")
            query = "select distinct ID from " & dbTables & "_Project where Category in ("
            first = True
            For Each item As String In temp
                If Not first Then
                    query = query & ","
        ...
        If Category.Length > 0 Then
            temp = Split(Category, ",")
            query = "select distinct ID from " & dbTables & "_Project where VS_Chevron in ("
            first = True
            For Each item As String In temp
                If Not first Then
                    query = query & ","
        ...
        If Category.Length > 0 Then
            temp = Split(Category, ",")
            query = "select distinct ID from " & dbTables & "_Project where Primary_Process in ("
            first = True
            For Each item As String In temp
                If Not first Then
                    query = query & ","
    ...
    Private Sub RawData()
        ...
        For Each rowR As DataRow In dataTableR.Rows
            For Each rowA As DataRow In dataTableA.Rows
                ...
                For column As Integer = 0 To 6
                    If column = 5 Then
                        dataTableF.Rows(rowF).Item(5) = translateRecipe(rowR.Item(column))
                    Else
                        dataTableF.Rows(rowF).Item(column) = rowR.Item(column)
                    End If
                Next
                dataTableF.Rows(rowF).Item(3) = getMonthlyFTE(rowR.Item(2), rowR.Item(4))
                For column As Integer = 0 To 4
                    dataTableF.Rows(rowF).Item(column + 7) = rowA.Item(column)
                Next
                rowF = rowF + 1
            Next
        Next
        ...
        For i As Integer = 0 To (dataTableF.Rows.Count - 1)
            If Not DotNet.IsEmpty(dataTableF.Rows(i).Item("Resource_ID")) Then
